...
def train_other_convention(genomes, config):
    ...
    if not master:
        ...
        if fitness >= 6 and pretender.id != master.id:
            for i in range(45):
...
def save_neat_as_coefficients(neat, name):
    biases = []
    for key in neat[0].nodes:
        biases.append(neat[0].nodes[key].bias)
    coefficients = []
    for evaluation in neat[1].node_evals:
        coefficients.append([])
...
if __name__ == '__main__':
    ...
    save_neat_as_coefficients(master, "master_coefs.txt")
